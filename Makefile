...
KIND_CLUSTER ?= janus-ingress-controller-test-e2e
...
build: manifests generate fmt vet ## Build manager binary.
	go build -o bin/manager /cmd/janusctl/main.go

.PHONY: run
run: manifests generate fmt vet ## Run a controller from your host.
	go run ./cmd/janusctl/main.go
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name janus-ingress-controller-builder
	$(CONTAINER_TOOL) buildx use janus-ingress-controller-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm janus-ingress-controller-builder
	rm Dockerfile.cross
